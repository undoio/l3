...
# `make clean` will have to clean-up a list of these stale sources
# To build multiple programs, LOC will generate the headers in common include/
STALE_LOC_GEN_SRCS  := $(L3_INCDIR)/$(LOC_HEADER) $(L3_INCDIR)/$(LOC_TOKENS)
...
# Retain this in list of stale generated files for `clean` to rm
STALE_LOC_GEN_SRCS += $(SINGLE_FILE_C_PROGRAM_GENSRC)
...
LOC_GENERATED := $(SINGLE_FILE_C_PROGRAM_GENSRC)       \
    ...
    GENERATED_SRCS := $(filter %.c,$(LOC_GENERATED))
    GENERATED_OBJS := $(GENERATED_SRCS:%.c=$(OBJDIR)/%.o)

    $(SINGLE_FILE_C_PROGRAM_OBJS): $(GENERATED_OBJS)
    $(GENERATED_OBJS): $(LOC_GENERATED)
    $(TEST_C_CODE_BINS): $(GENERATED_OBJS)

    ifeq "$(BUILD_VERBOSE)" "1"
        $(info )
        $(info ---- Debug ----)
        $(info $$LOC_GENERATED = [${LOC_GENERATED} ])
        $(info $$TEST_C_CODE_BINS = [${TEST_C_CODE_BINS} ])
        $(info $$GENERATED_SRCS = [${GENERATED_SRCS} ])
        $(info $$TEST_C_CODE_BINS = [${TEST_C_CODE_BINS} ])
        $(info ... <- depends on ...)
...
# Rule will be triggered for objects defined to be dependent on $(LOC_GENERATED) sources.
